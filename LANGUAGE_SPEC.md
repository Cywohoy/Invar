...
향후 문법 후보에서 사용하는 `fn`의 예약어 지정은 해당 기능을 설계할
때 확정한다.
...
따라서 적어도 정수 리터럴과 `Int` 변수 참조를 허용한다. 향후 정수
산술식을 구현하면 문자열 길이에도 같은 식을 사용할 수 있어야 한다.
...
일반 배열 전체는 하나의 입력 토큰으로 읽을 수 없다. 다만 원소 타입이
정확히 `String[1]`이면 각 원소를 한 바이트로 읽는 방법이 정해져
있으므로 배열 전체를 공백 없는 토큰 하나로 읽을 수 있다.

```invar
val n: Int[1..=100];
input { n; }
val word: Array[String[1], n];
input { word; }
```

위 입력은 바이트 길이가 정확히 `n`인 토큰 하나를 읽고 각 바이트를
길이 1의 `String` 원소에 차례대로 저장한다. UTF-8 문자 수가 아니라
바이트 수를 사용한다. 다른 배열은 `input { values[i]; }`처럼 원소
단위로 입력한다. `line(array)`는 현재 지원하지 않는다.
...
선언과 향후 함수 서명에는 타입을 항상 명시한다. 다만 컴파일러는
...
현재 토큰 단위 입력 패턴은 변수 이름 또는 배열 원소 접근이다.
...
서로 다른 기본 타입 사이의 비교와 `Unit`의 동등 비교는 허용하지
않는다. `&&`와 `||`는 C++와 마찬가지로 왼쪽부터 평가하며
short-circuit한다.
...
형태에서는 `else`를 생략할 수 있으며 마지막 세미콜론을 쓰지 않는다.
`else if`의 각 분기도 같은 조건을 만족해야 한다.
...
`for`는 항상 statement이고 블록 뒤에 세미콜론을 쓰지 않는다.
...
`while` 뒤에도 세미콜론을 쓰지 않는다. `break`, `continue`, for
이터레이터 문법은 아직 지원하지 않는다.
...
입력을 반복하는 본문은 매 반복이 끝날 때 시작 시점과 같은 줄 경계
상태여야 한다. 예를 들어 줄 중간까지만 읽고 다음 반복으로 넘어가는
본문은 오류다.

## 7. 향후 함수

사용자 정의 함수도 향후 지원한다. 매개변수와 반환 타입은 항상
명시하며 함수 본문, 재귀와 평가 규칙은 아직 결정하지 않았다.
...
- 배열 길이는 선언 시점에 값이 준비되어 있고 정적으로 비음수임을
  증명할 수 있어야 한다.
- `Array[String[1], n]`만 배열 전체를 토큰 하나로 입력할 수 있다.
- statement 형태의 `if`, `for`, `while` 뒤에는 세미콜론을 쓰지
  않는다.
...
- 세부 오류와 경고 메시지
- CLI 및 npm 배포 방식
- 사용자 정의 함수의 전체 문법 및 의미
- for 이터레이터, `break`, `continue`
